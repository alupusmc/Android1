package com.example.iopensopositivoAxel
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        texto.text = "esto es un mensaje positivo (+)"
        Toast.makeText( this,  "Io Penso Postivo", Toast.LENGTH_LONG).show()
        Log.i(   "edu.daec.iopenpositivo",  "IoPP Loading")
    }


    fun aleatorio( view: View){
        texto.text = navegar(3)
    }

    fun navegar(i: Int) :  String {

        when (i) {
            1 -> {
                posicion++
                if (posicion >= database.size) {
                    return database[database.size-1]
                } else {
                    return database[posicion]
                }
            }
            2 -> {
                posicion--
            ...
            3 -> {
                return database[Random.nextInt(from = 0, until = database.size - 1)]
            }
            4 -> {
                if (posicion < 0) {
                    return database[0]
                } else if (posicion >= database.size) {
                    return database[database.size-1]
                } else{
                    return database[posicion]
    ...
    fun adelante(view: View){
        texto.text = navegar(1)
    }

        fun atras(view: View){
            texto.text =    navegar(2)
        }

    fun buscar(view: View){
        posicion = editText.text.toString().toInt()
        texto.text =  navegar(4)
    }
